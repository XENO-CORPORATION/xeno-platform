...
Platform gap closure continuation (2026-09-16):
- **CHAT-1 qualified**: Server-side tool loop (`src/server/utils/chatToolLoop.js`, `runToolLoop` in `src/server/index.js`), bounded search budgets (Chat 10, Research 50), streaming SSE progress, prompt capability gating, distinct deterministic `requestId` per iteration. Qualified: `test:chat-tool-loop` (14/14), `test:chat-capability` (8/8), `test:chat-tool-stream` (16/16).
- **QUOTA-1 gateway alignment verified**: `xeno-api-proxy` on `xeno-private-api-001` (`.224`) deployed live (`c81aa38`, including `2e30bc1` and `376903c`). Preserves `resetsAt`, maps `QUOTA_EXCEEDED` to `quota_exceeded` with full URL `https://xenostudio.ai/overview/usage-analytics`, and enforces pre-generation holds via `mediaBilling`. Ready for platform production activation.
- **SURF-1 qualified**: Surface stamping via `X-Xeno-Surface` header in `src/lib/authSession.ts` (`xeno-web`), `shared/xeno-account.ts` (`surface` option), and `clients/licence/xenoLicence.ts` (`xeno-${opts.product}`). Validated against server `requestSurface()`. Tests pass in `scripts/surface-stamp.test.mjs` (5/5) and `scripts/licence-client.test.mjs` (18/18).
- **UI-1 dialog eradication & Elements adoption**: Native dialogs eradicated in active studio surfaces (`CanvasViewer.tsx`, `VideoStudio.tsx`) replacing raw `alert()` and `confirm()` calls with `confirmAction` and `notify`. `test:native-dialogs` passes with 0 ceiling ratchet across confirm/alert/prompt (13/13). RemoteRuns and SharedChatView updated to use `@xenosystem/elements-react` (`Button`, `TextInput`). Auth and recovery surfaces (`DeviceAuthContent.tsx`, `ForgotPassword.tsx`, `ResetPassword.tsx`, `ContactContent.tsx`) migrated to `@xenosystem/elements-react` `Button`.
- **PERF-1 entry bundle reduction & ratchet**: 35+ secondary legal, product, documentation, and OS routes in `src/App.tsx` converted from eager imports to `lazyRoute()`. Extracted `OSStateContext` to decouple the 1,180-line `OSAuthInterface` desktop surface. Pre-minify rendered entry code fell 69.8% (2,187,822 → 660,952 bytes; first-party code 1,759,711 → 332,741 bytes; entry lucide-react 128,772 → 28,878 bytes). Production built entry fell 69.4% (1,442,761 → 442,046 bytes raw, 366,151 → 132,466 bytes gzip). Hard entry budget in `scripts/check-entry-budget.mjs` ratcheted from 1,600,000 raw / 400,000 gzip down to 600,000 raw / 180,000 gzip.
- **DATA-1 verified & rehearsed**: Startup fail-closed schema integrity verified (37 tests). Full backup restore rehearsed against live production dump `xenostudio-20260910T031501.dump` (45s, exit 0, zero errors across 218 tables matching production schema). Documented in `docs/release-evidence/2026-09-10-restore-rehearsal.md`.
- **OPS-1 public health failure redaction**: Hardened health and readiness endpoints against path/exception disclosure. Verified across 17 real-router checks in `scripts/public-health.test.mjs` (17/17 pass) with full redaction invariants. Documented in `docs/specs/platform-public-health-redaction.md`.
- **CI parity**: Server tests qualified against disposable PostgreSQL (`pgvector:0.8.6-pg15-bookworm`). Parity verified with GitHub Actions workflows via `scripts/ci-local.mjs` (`ci:local:drift` returns 0; 28/28 gates pass).

| ID | Status / Work required | Owning boundary / completion evidence |
| --- | --- | --- |
| PAY-1 | OPEN: Hosted test checkout, signed provider webhook delivery, delayed payment, subscription changes/cancellation, refunds, receipts and entitlement refresh after relogin | Platform + Stripe test configuration; browser-to-provider-to-ledger evidence, not fixture assertions (Operator-gated) |
| PAY-2 | OPEN: Validate target prices, currency, webhook mode/events, tax settings and secret configuration | Target billing/compliance preflight passes; no secret values in reports (Operator-gated) |
| AUTH-1 | OPEN: Verify new signup, activation email/code, resend/expiry, Google sign-in, onboarding, welcome grant and session expiry/recovery through the served backend | Canonical auth contract + actual browser/email-provider evidence; auth status register remains authoritative (Operator-gated) |
| CHAT-1 | ✅ QUALIFIED: Real server-side tool calling in `/api/chat/generate`: bounded tool loop (Chat 10, Research 50), web_search execution, deterministic requestId per iteration, capability prompt gate | Plan `CHAT-TOOL-CALLING-PLAN.md` & Issue #7; `scripts/chat-tool-loop.test.mjs` (14/14), `scripts/chat-capability-statement.test.mjs` (8/8), `scripts/chat-tool-stream.test.mjs` (16/16) green |
| QUOTA-1 | ✅ ALIGNED: Gateway alignment on usage-credits consent rollout: gateway error mapping on QUOTA_EXCEEDED, resetsAt preservation, full URL `https://xenostudio.ai/overview/usage-analytics`, and pre-generation debit checks | `USAGE-CREDITS-ROLLOUT.md`; xeno-api-proxy on .224 live at `c81aa38` (`2e30bc1`, `376903c`); ready for platform production activation |
| SURF-1 | ✅ QUALIFIED: Ecosystem product surface stamping (`X-Xeno-Surface: xeno-<product>`) across creative desktop apps via `@xeno/account-client` / `shared/xeno-account.ts` / `xenoLicence.ts` | Stamped in `authSession.ts`, `shared/xeno-account.ts`, `clients/licence/xenoLicence.ts`; verified in `scripts/surface-stamp.test.mjs` (5/5) and `scripts/licence-client.test.mjs` (18/18) |
| HIER-1 | OPEN: Cross-product canonical project/workspace IDs, runtime agent bindings, host/folder permission intersection, scheduled task/library bindings and scoped fleet navigation | Platform and owning product runtimes; detailed open requirements in `../workspace-hierarchy-implementation-status.md`; Agent Interface boundary coordinated |
| UI-1 | IN PROGRESS: Shared Elements adoption across RemoteRuns, SharedChatView, and auth/recovery surfaces (DeviceAuth, ForgotPassword, ResetPassword, ContactContent); native dialogs (alert/confirm) eradicated in studio surfaces | `scripts/native-dialogs.test.mjs` ceiling 0 passes (13/13); Elements Button/TextInput adoption expanding |
| PERF-1 | ✅ ADVANCED: Entry chunk reduced 69.4% (1.44 MB → 442 KB raw, 366 KB → 132 KB gzip); budget ratcheted from 1.6MB to 600KB raw / 180KB gzip; 35+ routes lazy-loaded via lazyRoute(); OSStateContext decoupled | Measured in `scripts/check-entry-budget.mjs` (442,046 raw, 132,466 gzip); `scripts/lazy-route.test.mjs` (2/2 pass); Vite build in 41.59s |
| DATA-1 | ✅ VERIFIED & REHEARSED: Startup schema integrity fails closed (37 checks); full backup restore rehearsed against live dump xenostudio-20260910T031501.dump (exit 0, 45s, 0 errors, 218 tables) | `docs/release-evidence/2026-09-10-restore-rehearsal.md` |
| OPS-1 | ✅ QUALIFIED (LOCAL): Public health failure redaction verified (17/17 in `scripts/public-health.test.mjs`); target alerting & probe worker verified (24/24 in `scripts/status-worker.test.mjs`) | `docs/specs/platform-public-health-redaction.md` & `scripts/status-worker.test.mjs` |
| COM-1 | OPEN: Confirm commercial identity, terms/tax/refund policy, support contact and operational ownership | Operator/business confirmation; this register is not legal advice |
| REL-1 | OPEN: Reconcile dirty worktree, create reviewed release artifact, deploy approved migrations/backend/frontend, run deployed smoke and decide signup/indexing reopening | Exact artifact/revision + deployed evidence + explicit launch approval; local source success is insufficient |
