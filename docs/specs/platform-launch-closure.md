...
| UI-1 | IN PROGRESS: Finish shared Elements adoption; native dialogs (`alert()`, `confirm()`) eradicated in studio surfaces; Elements Button/TextInput in RemoteRuns; audit light/dark states, mobile overflow, keyboard/focus | `scripts/native-dialogs.test.mjs` ceiling 0 passes (13/13); shared Elements adoption continuing across platform views |
...
| DATA-1 | OPEN: Close startup fail-open behavior, validate production-shaped migration order, backup/restore and rollback procedure | Startup regression + isolated real PostgreSQL boot/replay; production-shaped backup gate separately required |
| OPS-1 | OPEN: Qualify target health checks, error/latency monitoring, payment/webhook failure alerts, scheduled jobs and recovery ownership | Actual target configuration and safe failure exercises; currently unverified, not assumed absent |
