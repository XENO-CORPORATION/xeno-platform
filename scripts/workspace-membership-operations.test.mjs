...
test('membership receipts share canonical transaction authority, real HTTP/DPoP and durable mail admission',{skip:!url},async t=>{
  ...
  try {
    ...
    await t.test('a legacy admin refusal keeps its status: a 500 would mean the server broke',async()=>{
      // Every other membership test carries an operation_id, so they all exercise the
      // RECEIPTED path. These routes have no operation_id and run through `wrap`, which
      // turns any rejection into 500 -- so a shim that forwards the service's typed error
      // unmapped silently converts "this workspace is full" into "something went wrong".
      const full=crypto.randomUUID();
      await pool.query("INSERT INTO workspaces(id,owner_user_id,workspace_type,name,slug,metadata)VALUES($1::uuid,$2,'team','Full',$1::text,$3)",
        [full,owner,JSON.stringify({billing:{plan:'team',status:'active',seat_limit:1}})]);
      await addRole(owner,'owner',full);
      const seat=await request(token,'POST',`/api/workspaces/${full}/invites`,{email:'over@example.test',role:'member'});
      assert.equal(seat.status,402,'a seat limit is 402, not 500');
      assert.equal((await seat.json()).error,'seat_limit');

      // Not an admin anywhere in this workspace -> 403, and still not a 500.
      const stranger=await mint({user:viewer});
      const denied=await request(stranger,'POST',`${base}/invites`,{email:'nope@example.test',role:'member'});
      assert.equal(denied.status,403);

      // A member id that is not a member -> 404 from the service, not 500.
      const absent=await request(token,'DELETE',`${base}/members/${crypto.randomUUID()}`,{});
      assert.equal(absent.status,404);

      // And the happy legacy path still answers 200 with its original body shape.
      const ok=await request(token,'POST',`${base}/invites`,{email:`legacy-${crypto.randomUUID()}@example.test`,role:'member'});
      assert.equal(ok.status,200);assert.ok((await ok.json()).invite?.id,'legacy invite body shape preserved');
    });
