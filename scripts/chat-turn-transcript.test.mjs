...
try {
  ...
  check('the edit field sits BARE in its card — no inner plate, no ring, no accent border (2026-09-18)', !chat.includes('border-[var(--chat-accent)]/70') && !chat.includes('focus-within:ring-1 focus-within:ring-[var(--chat-accent)]') && !chat.includes('rounded-lg border border-[var(--chat-border)] bg-[var(--chat-canvas)]/40 px-2.5 py-2') && (chat.match(/className="focus-self[^"]*resize-y[^"]*border-none/g) || []).length === 1 && chat.includes('resize-y border-none bg-transparent px-2.5 py-2'));
