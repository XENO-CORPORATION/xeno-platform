...
try {
   ...
   */
  check('ONE element from "Working for" to "Worked for": the thinking placeholder is rendered by the assistant block, not by an early return', !/if \(message\.isThinkingPlaceholder\) \{/.test(chat) && !/isThinkingPlaceholder && message\.id === aiRefinementPlaceholderId/.test(chat) && /message\.isStreaming \|\| message\.isThinkingPlaceholder \|\| message\.isDotPlaceholder\)/.test(chat));
  ...
  // 2026-09-26: parsed once into `storedTurn`, because the reload also reads which images the turn drew
  check('a stored message reads its turn back', /const storedTurn = isAi \? normalizeStoredTurn\(/.test(chat) && /turn: storedTurn,/.test(chat));
  ...
  check('a reasoning model that returned no trace shows NO thought box — the phantom "[Thinking process not provided…]" is gone', !/Thinking process not provided or markers not found/.test(chat) && (chat.match(/localHasThinking = true;/g) || []).length === 1 && /if \(streamedOrCarried\) \{\s*thinking = streamedOrCarried;\s*localHasThinking = true;/.test(chat) && /\) : null\}/.test(chat));
