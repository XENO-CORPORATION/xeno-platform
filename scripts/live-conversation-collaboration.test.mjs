...
test('live conversation collaboration is durable, sender-bound, race-safe and execution-isolated', { skip: !url }, async (t) => {
  ...
  try {
    ...
    await pool.query((await readFile(new URL('../src/server/database/migrations/20260918210000-chat-message-branches.sql', import.meta.url), 'utf8')).split('-- DOWN')[0]);
    ...
    const chainMessages = async (conversationId) => {
      await pool.query(
        `UPDATE chat_messages m SET parent_id = p.prev_id
           FROM (SELECT id, LAG(id) OVER (PARTITION BY conversation_id ORDER BY message_index, created_at) AS prev_id
                   FROM chat_messages WHERE conversation_id = $1) p
          WHERE m.id = p.id AND p.prev_id IS NOT NULL`,
        [conversationId],
      );
      await pool.query(
        `UPDATE chat_conversations c SET active_leaf_id = l.id
           FROM (SELECT id FROM chat_messages WHERE conversation_id = $1
                  ORDER BY message_index DESC, created_at DESC LIMIT 1) l
          WHERE c.id = $1`,
        [conversationId],
      );
    };
    const makeConversation = async (title = 'Live proof') => {
      ...
      await chainMessages(conversation.id);
    ...
    await t.test('public token preview is sanitized and token material is never stored', async () => {
      ...
      await chainMessages(conversationId);
