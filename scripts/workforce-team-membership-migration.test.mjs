...
test('team membership and admitted member snapshots on owned isolated PostgreSQL', async t => {
  ...
  const addMember = (resource = memberAgent, teamId = team, role = 'member') => pool.query(
  ...
  try {
    ...
    await t.test('membership creation advances the team revision and stamps join and change together', async () => {
      ...
      assert.equal(membership.role, 'member'); assert.equal(membership.created_by_user_id, creator);
    ...
    await t.test('one live membership per member, and endpoints must be exactly one kind', async () => {
      ...
      await assert.rejects(pool.query(
        `INSERT INTO workforce_team_memberships(team_id,member_resource_id,member_resource_kind) VALUES($1,$2,'agent')`,
        [team, team]), error => error.code === '23503');
      await deny(`INSERT INTO workforce_team_memberships(team_id,member_resource_id,member_resource_kind,member_principal_id)
         VALUES($1,$2,'agent',$3)`, [team, loneAgent, human]);
    });
    ...
    await t.test('a human member can be erased, leaving a tombstone and the membership intact', async () => {
      const { rows: [m] } = await pool.query(
        `INSERT INTO workforce_team_memberships(team_id,member_principal_id) VALUES($1,$2) RETURNING *`, [backfilledTeam, human]);
