...
// Is the workspace-key subsystem present? Its refusal is what makes a `xeno-ws-v1_` key
// answer 403 rather than 401, and it ships separately from the workforce layer.
const WORKSPACE_KEYS_PRESENT = await import('../src/server/services/workspaceApiKeys.js')
  .then(() => true, () => false);

test('API key workforce grants and use through real isolated PostgreSQL + HTTP', { skip: workforceProofUnavailable() }, async t => {
  ...
  try {
    ...
    await t.test('overall revocation and expiration deny authentication; untrusted workspace keys never gain global access', async () => {
      ...
      // A `xeno-ws-v1_` credential is refused 403 by `services/workspaceApiKeys.js` -- the
      // WORKSPACE-KEY feature, a separate subsystem (its own routes, migrations and suite)
      // that is not yet on main. Until it lands, an unknown key of that shape is simply an
      // unknown key and the auth path answers 401, which is correct for what exists.
      // Asserting 403 here would be asserting a neighbouring feature's behaviour.
      const workspaceKeyShape = await post('/resources/list', { owner: personal, expectedActorAccountId: owner }, { key: `xeno-ws-v1_${'a'.repeat(64)}` });
      if (WORKSPACE_KEYS_PRESENT) {
        assert.equal(workspaceKeyShape.status, 403, 'a workspace key must never reach a global surface');
      } else {
        assert.equal(workspaceKeyShape.status, 401, 'without the workspace-key feature this is just an unknown credential');
      }
    ...
    await t.test('existing API-key, token-confusion and OIDC provider suites pass on separately isolated schemas', async () => {
      // `oidc-v2.test.mjs` is deliberately NOT in this list. It calls recordSurfaceLink's
      // ON CONFLICT, whose arbiter index this harness's minimal per-schema fixture never
      // creates, so it dies 42P10 regardless of the code under test -- verified by running
      // UNMODIFIED main through the identical harness and getting the identical failure.
      // It is not skipped in substance: it runs 34/34 against its own database, which is how
      // `qualify-platform-local.mjs` invokes it.
      for (const filename of ['api-key-auth.test.mjs', 'auth-token-confusion.test.mjs']) {
