...
description: "Release a XENO product to Cloudflare R2 + xenostudio.ai + XENO Hub. Invoke when the user wants to publish a new version (desktop installer or CLI/npm), cut a patch or hotfix, regenerate a product's release feed, bootstrap a new product's feed, or deploy landing/docs changes to the site. Routes to the correct path and follows the repo's release-guide/. NOT for local package publishability checks — that is the separate built-in Release skill."
enabled: true
visibility: visible
---
