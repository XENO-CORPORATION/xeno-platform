...
import { authorityTransaction } from '../services/workspaceOperationReceipts.js';
import { createAccountWorkspace, provisionPersonalWorkspace, decideWorkspaceInvite } from '../services/workspaceLifecycle.js';
...
const wrap = (fn) => (req, res) => Promise.resolve(fn(req, res)).catch((e) => {
  ...
  if (!res.headersSent) res.status(e.status || 500).json({ success: false, error: e.status ? e.code || e.message : 'Internal error' });
});
...
// Legacy requests remain non-replayable but use exactly the same transaction and
// business rules as identified requests. The delivery worker sends committed mail.
async function legacyMembership(req, res, action, input) {
  const { result } = await mutateWorkspaceMembership(req.db, req.params.id, req.user.id, null, null, action, input, { legacy: true });
...
router.get('/', wrap(async (req, res) => {
  await provisionPersonalWorkspace(req.db, req.user);
...
router.post('/', wrap(async (req, res) => {
  const type=req.body?.workspace_type ?? 'team';
  const ins=await authorityTransaction(req.db,db=>createAccountWorkspace(db,req.user.id,req.body?.name,type));
...
router.patch('/:id/members/:memberId', wrapId((req,res)=>legacyMembership(req,res,'member.role',{memberId:req.params.memberId,role:toDisplayRole(toRelation(req.body?.member_role))})));
// DELETE /api/workspaces/:id/members/:memberId  (admin removes; or self-leave)
router.delete('/:id/members/:memberId', wrapId((req,res)=>legacyMembership(req,res,'member.remove',{memberId:req.params.memberId})));
// POST /api/workspaces/:id/owner-transfer { new_owner_user_id }
router.post('/:id/owner-transfer', wrapId((req,res)=>legacyMembership(req,res,'owner.transfer',{newOwnerId:req.body?.new_owner_user_id})));
...
router.post('/:id/invites', wrapId((req,res)=>legacyMembership(req,res,'invite.create',{email:String(req.body?.email||'').trim().toLowerCase(),role:toDisplayRole(toRelation(req.body?.role))})));
// DELETE /api/workspaces/:id/invites/:inviteId  (revoke)
router.delete('/:id/invites/:inviteId', wrapId((req,res)=>legacyMembership(req,res,'invite.revoke',{inviteId:req.params.inviteId})));
// POST /api/workspaces/:id/invites/:inviteId/resend
router.post('/:id/invites/:inviteId/resend', wrapId((req,res)=>legacyMembership(req,res,'invite.resend',{inviteId:req.params.inviteId})));
...
inviteRouter.get('/', wrap(async (req, res) => {
  const rows = (await req.db.query(
    `SELECT i.*, w.name AS workspace_name, u.display_name AS invited_by_name
       ...
       JOIN workspaces w ON w.id = i.workspace_id
       LEFT JOIN users u ON u.id = i.invited_by_user_id
      WHERE i.status = 'pending' AND w.status = 'active' AND (i.expires_at IS NULL OR i.expires_at > now())
        AND (i.invited_user_id = $1 OR (i.invited_user_id IS NULL AND lower(i.invited_email) = lower($2)))
      ORDER BY i.created_at DESC`,
...
// No operation ID means no promise of replay. Commit is complete before ACK.
for (const action of ['accept','decline']) inviteRouter.post(`/:inviteId/${action}`,wrap(async(req,res)=>{
  const result=await authorityTransaction(req.db,db=>decideWorkspaceInvite(db,req.user.id,req.params.inviteId,action));
