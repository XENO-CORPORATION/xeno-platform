...
const TAGS = [
  ...
  { namespace: 'topic', value: 'clipboard' },
  { namespace: 'topic', value: 'shortcuts' },
  { namespace: 'topic', value: 'css' },
  { namespace: 'product', value: 'post' },
];
...
const THREADS = [
  ...
  },
  {
    ...
    tags: ['product:pixel', 'kind:bug', 'topic:clipboard', 'topic:electron'],
    body: [
      '"This is the original image, but this is the one imported — the colors look totally different. Blue robot turned orange/yellow, red demon turned blue."',
      '',
      '**Affects:** Electron apps using `clipboard.readImage().toBitmap()` on Windows or macOS.',
    ].join('\n'),
    answer: [
      '**`nativeImage.toBitmap()` does NOT return RGBA on all platforms.** Linux gives RGBA; Windows and macOS give **BGRA**.',
      '',
      'Treat the returned Buffer as RGBA — wrap it in a `Uint8ClampedArray` and hand it to `new ImageData()` — and every red and blue swaps. A pure-blue pixel `[0, 0, 255, 255]` read as RGBA renders as red, and vice versa.',
      '',
      'This catches people because the Electron docs say "RGBA-formatted Buffer", while in practice the byte order matches whatever the OS uses internally.',
      '',
      '**The fix:** use `nativeImage.toPNG()` on the main side and decode the PNG in the renderer. The PNG format carries its own channel order, so nothing has to be guessed.',
    ].join('\n'),
  },
  {
    key: 'ctrl-t-first-press',
    space: 'help',
    title: 'Free Transform (Ctrl+T) "does nothing" on the first press but works on the second',
    tags: ['product:pixel', 'kind:bug', 'topic:shortcuts'],
    body: [
      '"I press Ctrl+T and nothing happens. I press it again and now the bounding box appears."',
      '',
      '**Affects:** any tool whose shortcut needs BOTH a global state change and component-local context — overlay refs, layer data, viewport maths.',
    ].join('\n'),
    answer: [
      'TWO bugs stacked, and either alone would have been easier to find.',
      '',
      '**1. A silent guard.** The Background layer cannot be Free-Transformed, and that rule was enforced as a bare early return: `if (getEffectiveLocks(activeLayer.id).position) return`. In a new document the active layer IS the Background, so the first press hit the guard and returned — no toast, no prompt, nothing. The key felt dead.',
      '',
      '**2. Two keydown handlers competing for one shortcut.** A global hook registered Ctrl+T and only switched the active tool; the viewport registered its own Ctrl+T that did the real work. Both were on `window`. The global one triggered a store update mid-dispatch, which rebound the viewport handler between presses — so press #1 switched the tool and press #2 did the work.',
      '',
      '**The lesson worth carrying:** a guard that returns silently is indistinguishable from a broken feature. Refuse loudly, or do not refuse.',
    ].join('\n'),
  },
  {
    key: 'delete-key-noop',
    space: 'help',
    title: 'Pressing Delete with a layer or selection does nothing',
    tags: ['product:pixel', 'kind:bug', 'topic:shortcuts'],
    body: [
      '"When I select a layer and press Delete it does not delete it." Nothing happens at all.',
      '',
      '**Affects:** any app with a global keyboard hook where an expected key was simply never added.',
    ].join('\n'),
    answer: [
      '`Delete` and `Backspace` were **never bound** in the global keyboard hook. The only Delete handlers anywhere were text-cursor editing, gated behind an active text layer — so with a layer or selection active the key was an unhandled no-op. `deleteLayer()` existed, wired only to the trash button and the context menu.',
      '',
      '**The fix binds the intent, not the literal ask.** In Photoshop, Delete with an active selection clears the selected pixels; deleting a whole layer is the trash button. So Delete/Backspace now clears selection contents — undoable, respecting pixel locks — and is a **no-op without a selection**, so the key can never silently destroy a layer.',
      '',
      'Guarded against text-edit, transform, crop and in-progress interactions.',
    ].join('\n'),
  },
  {
    key: 'button-cursor-arrow',
    space: 'help',
    title: 'Buttons show the arrow cursor instead of the pointer hand',
    tags: ['product:post', 'kind:bug', 'topic:css', 'topic:build'],
    body: [
      'Buttons — especially inside modals and popups — do not show the pointer-hand cursor on hover. Clicks work, but the interface reads as unresponsive.',
      '',
      '**Affects:** anything on Tailwind v4 with a CSS-in-Rust bundler (Turbopack / Lightning CSS).',
    ].join('\n'),
    answer: [
      'Native `<button>` defaults to `cursor: default`, not `pointer` — unlike `<a>` — and Tailwind\'s Preflight reset does not add one.',
      '',
      '**The compounding cause is the one that wasted the time.** The first fix was a global `@layer base` reset that used a `:has()` selector and a long `:not()` chain, and Turbopack / Lightning CSS **silently dropped the entire rule block at build time**. No error, no warning. It looked like a specificity problem and was a build-time deletion.',
      '',
      '**Fix, both halves required:** a global base reset using SIMPLE selectors only (no `:has()`, no multi-level `:not()`), AND `cursor-pointer` baked directly into every button component class so it survives even if the base layer is dropped.',
      '',
      '**"My global CSS rule is not applying" → check the COMPILED stylesheet for the rule before debugging specificity.** Build tools can drop rules; `curl <css-chunk> | grep cursor` proved it was absent rather than overridden.',
    ].join('\n'),
  },
];
