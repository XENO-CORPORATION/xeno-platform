...
--
-- ONE-WAY, DELIBERATELY: this migration has no rollback section and cannot have an honest
-- one. Dropping the tables and columns would be trivial, but the statement below NULLs every
-- stored share_token, and a plaintext capability cannot be recovered from its SHA-256 digest.
-- A rollback that restored the schema while leaving those columns empty would report an
-- undo that did not happen.
--
-- The tokens are not lost to the people holding them: a share link carries the token itself,
-- and token_digest is already the only value any lookup consults (chatRoutes matches on it in
-- both share reads), so existing links keep working. What is deleted is OUR copy -- which is
-- the point. After this, a database disclosure no longer yields a set of working share links.
