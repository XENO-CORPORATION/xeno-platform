...
const ImageGenerationInterface2: React.FC = () => {
  ...
  const [showDesktopLibrarySearch, setShowDesktopLibrarySearch] = useState(false);
  ...
  const [desktopSettingsInlineVisible, setDesktopSettingsInlineVisible] = useState(false);
  const [desktopSettingsInlineClosing, setDesktopSettingsInlineClosing] = useState(false);
  const [desktopAiCompaniesVisible, setDesktopAiCompaniesVisible] = useState(false);
  ...
  const [desktopLibrarySearchWidth, setDesktopLibrarySearchWidth] = useState(220);
  const [desktopModelShrinkHold, setDesktopModelShrinkHold] = useState(false);
  const [desktopSearchModelCarryover, setDesktopSearchModelCarryover] = useState(false);
  const [desktopSearchPromptCarryoverCap, setDesktopSearchPromptCarryoverCap] = useState<number | null>(null);
  const modelHoverTimeoutRef = useRef<number | null>(null);
  const modelHoverLockUntilRef = useRef(0);
  ...
  // Track mobile and windowed/narrow viewport states for responsive header behavior.
  useEffect(() => {
    const checkViewportState = () => {
      const width = window.innerWidth;
      const mobile = width < 768;
      const desktopWindowed = !mobile && width < 1280;

      setViewportWidth(width);
      setIsMobile(mobile);
      setIsViewportConstrained(mobile || desktopWindowed);

      // Auto-close model flyout and search bar when viewport gets too narrow
      if (desktopWindowed && width < 960) {
        setShowAiCompanies(false);
        setSelectedCompany(null);
        if (showDesktopLibrarySearch) {
          setShowDesktopLibrarySearch(false);
          setLibrarySearchQuery('');
          setDesktopSearchModelCarryover(false);
          setDesktopSearchPromptCarryoverCap(null);
        }
      }
    };
  ...
  useEffect(() => {
    if (isMobile) {
      setDesktopSettingsInlineVisible(false);
      setDesktopSettingsInlineClosing(false);
      setShowDesktopLibrarySearch(false);
      return;
    }

    if (showSettings) {
      setDesktopSettingsInlineVisible(true);
      setDesktopSettingsInlineClosing(false);
      return;
    }

    if (!desktopSettingsInlineVisible) return;

    setDesktopSettingsInlineClosing(true);
  ...
  }, [desktopSettingsInlineVisible, isMobile, showSettings]);

  useEffect(() => {
    if (isMobile) return;

    const updateDesktopLibrarySearchWidth = () => {
      const modelControlsNode = desktopModelControlsRef.current;
      ...
      if (!promptRect || !settingsRect || !searchSlotRect || !modelControlsNode) return;

      const headerGapPx = window.innerWidth >= 1024 ? 12 : 8;
      const availableWidth = settingsRect.left - promptRect.right - headerGapPx * 2;
      const minSearchSlot = window.innerWidth >= 1024 ? 48 : 40;
      setDesktopLibrarySearchWidth(Math.max(minSearchSlot, availableWidth));

      let leftOccupiedRight = modelControlsNode.getBoundingClientRect().right;
      const openPopouts = modelControlsNode.querySelectorAll<HTMLElement>('[data-desktop-header-popout="true"]');

      openPopouts.forEach((node) => {
        const rect = node.getBoundingClientRect();
        if (rect.width > 0 && rect.height > 0) {
          leftOccupiedRight = Math.max(leftOccupiedRight, rect.right);
        }
      });
    ...
    const runMeasuredUpdate = () => {
      window.requestAnimationFrame(updateDesktopLibrarySearchWidth);
    };

    runMeasuredUpdate();
    window.addEventListener('resize', runMeasuredUpdate);
    const delayedUpdateId = window.setTimeout(runMeasuredUpdate, 180);
    ...
    if (resizeObserver) {
      if (desktopModelControlsRef.current) {
        resizeObserver.observe(desktopModelControlsRef.current);
      }
      if (desktopPromptContainerRef.current) {
        resizeObserver.observe(desktopPromptContainerRef.current);
      }
      if (desktopSearchSlotRef.current) {
        resizeObserver.observe(desktopSearchSlotRef.current);
      }
      if (desktopSettingsButtonRef.current) {
        resizeObserver.observe(desktopSettingsButtonRef.current);
      }
    }

    return () => {
      window.removeEventListener('resize', runMeasuredUpdate);
      window.clearTimeout(delayedUpdateId);
      resizeObserver?.disconnect();
    };
  }, [desktopAiCompaniesMode, desktopAiCompaniesVisible, desktopSettingsInlineVisible, isMobile, isViewportConstrained, selectedCompany, selectedModel, showDesktopLibrarySearch]);

  useEffect(() => {
    if (!showDesktopLibrarySearch) return;
  ...
  useEffect(() => {
    if (isMobile) {
      setDesktopAiCompaniesVisible(false);
      setDesktopAiCompaniesClosing(false);
      return;
    }

    if (showAiCompanies) {
      setDesktopAiCompaniesVisible(true);
      setDesktopAiCompaniesClosing(false);
      if (modelHoverTimeoutRef.current) {
        window.clearTimeout(modelHoverTimeoutRef.current);
        modelHoverTimeoutRef.current = null;
      }
      setHoveredModel(null);
      return;
    }

    if (!desktopAiCompaniesVisible) return;
    ...
    return () => window.clearTimeout(timeoutId);
  }, [desktopAiCompaniesVisible, isMobile, showAiCompanies]);
  ...
  const filteredGenerations = useMemo(() => {
    ...
    return generations.filter((gen) => gen.prompt.toLowerCase().includes(normalizedLibrarySearchQuery));
  }, [generations, normalizedLibrarySearchQuery]);
  const libraryImages = filteredGenerations.flatMap((gen) =>
  ...
  const cardRadius = 17;
  const galleryGapPx = 16;
  const galleryMinItemWidthPx = 200;
  const galleryRowHeightPx = 256;
  const galleryMaxRowHeightPx = 310;
  ...
  const galleryItemLayout = useMemo(() => {
    ...
    // Calculate the row height if all items in the row fill the full width
    const getJustifiedHeight = (row: Array<{ key: string; aspect: number }>) => {
    ...
    // Build rows: keep adding items until the justified height drops to or below targetRowHeight
    const rows: Array<Array<{ key: string; aspect: number }>> = [];
    let currentRow: Array<{ key: string; aspect: number }> = [];

    gridItems.forEach((item) => {
      const aspect = Math.max(0.35, Math.min(3.0, resolveAspect(item)));
      currentRow.push({ key: item.key, aspect });

      const h = getJustifiedHeight(currentRow);
      // Row is full when the justified height drops to or below the target
      if (h <= targetRowHeight && currentRow.length >= 2) {
    ...
    // Assign pixel sizes per item
    rows.forEach((row, rowIndex) => {
      const isLastRow = rowIndex === rows.length - 1;
      const justifiedH = getJustifiedHeight(row);

      // For the last (possibly incomplete) row: if full justification would make tiles taller than max, cap it
      if (isLastRow && justifiedH > galleryMaxRowHeightPx) {
        const cappedH = targetRowHeight;
        row.forEach((entry) => {
          layout.set(entry.key, {
            width: Math.max(60, Math.floor(cappedH * entry.aspect)),
            height: Math.floor(cappedH),
          });
        });
        return;
      }

      // Fully justified: every row fills 100% of the container width, no empty space
      const gapsWidth = galleryGapPx * Math.max(0, row.length - 1);
      const availableForImages = effectiveWidth - gapsWidth;
      const aspectSum = row.reduce((sum, entry) => sum + entry.aspect, 0);
      const rowH = Math.floor(availableForImages / aspectSum);

      // Distribute widths precisely so the total equals effectiveWidth exactly (no sub-pixel gaps)
      let usedWidth = 0;
      row.forEach((entry, i) => {
        const isLast = i === row.length - 1;
        const idealW = rowH * entry.aspect;
        const w = isLast
          ? effectiveWidth - gapsWidth - usedWidth  // last item absorbs rounding remainder
          : Math.floor(idealW);
        usedWidth += w;
        layout.set(entry.key, { width: Math.max(60, w), height: rowH });
      });
    });
  ...
  const desktopPromptMaxWidth = useMemo(() => {
    const fluidPromptWidth = Math.round(viewportWidth * (isViewportConstrained ? 0.46 : 0.52));
    let maxWidth = Math.max(360, Math.min(835, fluidPromptWidth));
    const searchShrink = isViewportConstrained && showDesktopLibrarySearch ? 132 : 0;

    if (isViewportConstrained) {
      const modelShrink = (desktopAiCompaniesVisible || desktopAiCompaniesClosing || desktopModelShrinkHold || desktopSearchModelCarryover)
        ? desktopAiCompaniesMode === 'selected' ? 140 : 92
        : 0;
      const combinedOpenShrink = searchShrink > 0 && modelShrink > 0 ? 88 : 0;

      maxWidth -= searchShrink + modelShrink + combinedOpenShrink;
    }

    return Math.max(280, Math.min(maxWidth, desktopPromptCollisionLimit ?? maxWidth, desktopSearchPromptCarryoverCap ?? maxWidth));
  }, [desktopAiCompaniesClosing, desktopAiCompaniesMode, desktopAiCompaniesVisible, desktopModelShrinkHold, desktopPromptCollisionLimit, desktopSearchModelCarryover, desktopSearchPromptCarryoverCap, isViewportConstrained, showDesktopLibrarySearch, viewportWidth]);
  const visibleDesktopAiCompanies = useMemo(
    () => (desktopAiCompaniesMode === 'selected' && selectedModelCompany ? [selectedModelCompany] : aiCompanies),
    [desktopAiCompaniesMode, selectedModelCompany]
  );
  const clearModelHoverState = () => {
    if (modelHoverTimeoutRef.current) {
      window.clearTimeout(modelHoverTimeoutRef.current);
      modelHoverTimeoutRef.current = null;
    }
  ...
  const lockModelHover = (durationMs = 180) => {
    modelHoverLockUntilRef.current = Date.now() + durationMs;
    clearModelHoverState();
  };
  const handleModelHoverEnter = (model: ModelDetails) => {
    if (Date.now() < modelHoverLockUntilRef.current) {
      return;
    }
    if (modelHoverTimeoutRef.current) {
      window.clearTimeout(modelHoverTimeoutRef.current);
    }
    modelHoverTimeoutRef.current = window.setTimeout(() => {
      setHoveredModel(model);
      modelHoverTimeoutRef.current = null;
    }, 700);
  };
  const handleModelHoverLeave = () => {
    clearModelHoverState();
  };
  ...
  useEffect(() => {
    if (!desktopModelShrinkHold) return;

    const timeoutId = window.setTimeout(() => {
      setDesktopModelShrinkHold(false);
    }, 260);

    return () => window.clearTimeout(timeoutId);
  }, [desktopModelShrinkHold]);
    ...
    <div className="h-full w-full flex flex-col py-3 pb-36 md:pb-3 overflow-y-auto relative bg-[#111214]">
      ...
      <div className="w-full mx-auto relative flex flex-col flex-1">
        ...
        <div className="w-full hidden md:flex flex-col gap-3 relative">
          ...
          <div className="w-full hidden md:flex flex-row items-start justify-between gap-2 lg:gap-3 relative">
            ...
            >
              ...
              >
                {selectedModelCompany ? (
                  <span className="w-6 h-6 flex items-center justify-center text-white/70">
                    {selectedModelCompany.logo}
                  </span>
                ) : (
                ...
                <>
                  ...
                  <button
                    onClick={() => {
                      ...
                      setSelectedCompany(selectedModelCompany.name);
                      setShowSettings(false);
                      setActiveImageId(null);
                      setShowTools(false);
                      setSelectedTool(null);
                    }}
                    className="flex-1 flex items-center justify-center overflow-hidden px-1 rounded transition-all hover:bg-white/5"
                  >
            ...
            {desktopAiCompaniesVisible && (
              <>
                {desktopAiCompaniesMode === 'selected' && selectedModelCompany ? (
                  <div
                    data-desktop-header-popout="true"
                    className={`absolute left-full top-0 ml-2 hidden md:flex flex-row items-center gap-2 p-2 z-[110] ${desktopAiCompaniesClosing ? 'pointer-events-none' : 'pointer-events-auto'}`}
                    style={{
                      animation: `${desktopAiCompaniesClosing ? 'desktopAiCompaniesExit' : 'desktopAiCompaniesEnter'} 160ms ease-out both`,
                      animationDelay: `${desktopAiCompaniesClosing ? Math.max(selectedModelCompany.models.length - 1, 0) * 40 : 0}ms`,
                      transformOrigin: desktopAiCompaniesClosing ? 'right center' : 'left center',
                    }}
                  >
                    {selectedModelCompany.models.map((model, modelIndex) => (
                      <div
                        key={model.name}
                        className="relative"
                        ...
                        style={{
                          animation: `${desktopAiCompaniesClosing ? 'desktopAiCompanyItemExit' : 'desktopAiCompanyItemEnter'} 160ms ease-out both`,
                          animationDelay: `${desktopAiCompaniesClosing ? (selectedModelCompany.models.length - 1 - modelIndex) * 40 : modelIndex * 40}ms`,
                        }}
                      >
                        {hoveredModel && hoveredModel.name === model.name && (
                          <div
                            className="absolute left-0 top-full mt-2 w-56 bg-[#1a1a1c] backdrop-blur-md border border-[#3a3a3d] rounded-lg p-3 shadow-xl z-[120]"
                            style={{
                  ...
                  <div
                    data-desktop-header-popout="true"
                    className={`absolute left-full top-0 ml-2 hidden md:flex flex-row items-center gap-2 p-2 z-[100] ${desktopAiCompaniesClosing ? 'pointer-events-none' : 'pointer-events-auto'}`}
                    style={{
                      animation: `${desktopAiCompaniesClosing ? 'desktopAiCompaniesExit' : 'desktopAiCompaniesEnter'} 160ms ease-out both`,
                      animationDelay: `${desktopAiCompaniesClosing ? Math.max(visibleDesktopAiCompanies.length - 1, 0) * 40 : 0}ms`,
                      transformOrigin: desktopAiCompaniesClosing ? 'right center' : 'left center',
                    }}
                  >
                    {visibleDesktopAiCompanies.map((company, index) => (
                      <div
                        key={company.name}
                        className="relative"
                        style={{
                          animation: `${desktopAiCompaniesClosing ? 'desktopAiCompanyItemExit' : 'desktopAiCompanyItemEnter'} 160ms ease-out both`,
                          animationDelay: `${desktopAiCompaniesClosing ? (visibleDesktopAiCompanies.length - 1 - index) * 40 : index * 40}ms`,
                        }}
                      >
                        <button
                          onClick={() => {
                        ...
                        {selectedCompany === company.name && (
                          <div data-desktop-header-popout="true" className="absolute left-0 top-full mt-2 flex flex-col gap-1.5 z-[110]">
            ...
            */}
            ...
            >
            <div className="h-10 lg:h-12 backdrop-blur-md border border-[#3a3a3d] rounded-lg flex items-center justify-between relative shadow-lg shadow-black/40 bg-[#1a1a1c]" style={{ boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.2), 0 0 15px rgba(0, 0, 0, 0.3)' }}>
            ...
            <div
              ...
              className="relative h-10 lg:h-12 flex items-center justify-end min-w-0"
              style={{ width: `${desktopLibrarySearchWidth}px` }}
            >
              <div
                ...
                style={{
                  width: showDesktopLibrarySearch ? '100%' : '40px',
                  boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.2), 0 0 15px rgba(0, 0, 0, 0.3)',
                }}
              />
              <div className={`absolute inset-y-0 left-0 right-10 lg:right-12 flex items-center pl-3 pr-2 transition-opacity duration-150 ${showDesktopLibrarySearch ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                <div className="min-w-0 w-full">
                  <input
                    ref={librarySearchInputRef}
                    type="text"
                    value={librarySearchQuery}
                    onChange={(e) => setLibrarySearchQuery(e.target.value.slice(0, 500))}
                    onKeyDown={(e) => {
                      if (e.key === 'Escape') {
                        setShowDesktopLibrarySearch(false);
                        setLibrarySearchQuery('');
              ...
              <button
                onClick={() => {
                  ...
                  if (desktopAiCompaniesVisible || desktopAiCompaniesClosing) {
                    setDesktopModelShrinkHold(true);
                    setDesktopSearchModelCarryover(true);
                    const currentPromptWidth = desktopPromptContainerRef.current?.getBoundingClientRect().width;
                    setDesktopSearchPromptCarryoverCap(currentPromptWidth ? Math.round(currentPromptWidth) : null);
                  } else {
                    setDesktopSearchModelCarryover(false);
                    setDesktopSearchPromptCarryoverCap(null);
                  }

                  setShowDesktopLibrarySearch(true);
                }}
                className="absolute right-0 top-0 z-10 h-full w-10 lg:w-12 rounded-lg flex items-center justify-center transition-all hover:bg-white/5"
                aria-label="Toggle search"
              >
                <svg className={`w-5 h-5 transition-colors ${showDesktopLibrarySearch || librarySearchQuery ? 'text-white/80' : 'text-white/40'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div ref={desktopSettingsButtonRef} className="relative">
              ...
              {desktopSettingsInlineVisible && (
                <div
                  className={`absolute top-full right-0 mt-2 hidden md:flex w-max flex-row-reverse items-start gap-2 whitespace-nowrap z-[115] ${desktopSettingsInlineClosing ? 'pointer-events-none' : 'pointer-events-auto'}`}
                  style={{
                    animation: `${desktopSettingsInlineClosing ? 'desktopSettingsInlineExit' : 'desktopSettingsInlineEnter'} 160ms ease-out both`,
                  }}
                >
                  <div
                  ...
                  >
                    <button
                      ...
                      className="w-full h-8 px-3 bg-[#1a1a1c] backdrop-blur-md border border-[#3a3a3d] rounded-lg flex items-center gap-2 text-white/90 text-sm hover:bg-[#2a2a2d] hover:border-white/30 transition-all shadow-lg"
                      style={{ boxShadow: '0 4px 12px -2px rgba(0, 0, 0, 0.4), 0 0 20px rgba(0, 0, 0, 0.4)' }}
                    >
                    ...
                    {showResolutions && (
                      <div className="absolute top-full right-0 mt-2 w-40 bg-[#1a1a1c] backdrop-blur-md border border-[#3a3a3d] rounded-lg overflow-hidden shadow-xl shadow-black/30 z-[120] p-1" style={{ boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.4), 0 0 25px rgba(0, 0, 0, 0.5)' }}>
                  ...
                  >
                    ...
                    {showAspectRatios && (
                      <div
                        className="absolute top-full mt-2 bg-[#1a1a1c] backdrop-blur-md border border-[#3a3a3d] rounded-lg overflow-y-auto overflow-x-hidden shadow-xl shadow-black/30 z-[120] p-1"
                        style={{
            ...
            <div className="h-10 lg:h-12 backdrop-blur-md border border-[#3a3a3d] rounded-lg flex items-center justify-center px-1.5 lg:px-2 gap-0.5 lg:gap-1 shadow-lg shadow-black/40 relative bg-[#1a1a1c]" style={{ boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.2), 0 0 15px rgba(0, 0, 0, 0.3)' }}>
            {/* History Button */}
            <button
              onClick={() => {
                // Trigger animation
          ...
          <style>{`
            @keyframes desktopSettingsInlineEnter {
              from {
                opacity: 0;
            ...
            @keyframes desktopSettingsInlineExit {
              from {
                opacity: 1;
                transform: translateX(0);
              }
            ...
            @keyframes desktopAiCompaniesEnter {
              from {
                opacity: 0;
            ...
            @keyframes desktopAiCompaniesExit {
              from {
                opacity: 1;
                transform: scaleX(1);
              }
            ...
            @keyframes desktopAiCompanyItemEnter {
              from {
                opacity: 0;
                transform: translateX(-12px);
              }
            ...
            @keyframes desktopAiCompanyItemExit {
              from {
                opacity: 1;
                transform: translateX(0);
              }
        ...
        >
          ...
          <div className="flex flex-col-reverse md:flex-col gap-6 py-4">
                  ...
                  >
                    ...
                    {gridItems.map((item) => {
                      ...
                      const cardStyle: React.CSSProperties = {
                        ...
                        flex: '0 0 auto',
                        outline: '1px solid rgba(218, 220, 224, 0.1)',
                        outlineOffset: '0px',
                      ...
                      return (
                        ...
                        >
                          <img
                            ...
                            draggable={false}
                            className="w-full h-full object-cover block max-w-full select-none"
                            style={{
                              transition: 'scale 0.2s ease-in-out',
                              scale: '1',
                            }}
                            onMouseEnter={(e) => { (e.target as HTMLImageElement).style.scale = '1.02'; }}
                            onMouseLeave={(e) => { (e.target as HTMLImageElement).style.scale = '1'; }}
                          />
            ...
            {!isGenerating && generations.length > 0 && filteredGenerations.length === 0 && (
              <div className="flex flex-col items-center justify-center py-20 text-white/40">
                <svg className="w-16 h-16 mb-4 text-white/20" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <circle cx="11" cy="11" r="7" strokeWidth={1.5} />
                  <path strokeLinecap="round" strokeWidth={1.5} d="M20 20L16.65 16.65" />
                </svg>
                <p className="text-lg mb-2">No matching prompts found</p>
                <p className="text-sm text-white/30">Try searching with fewer words or a different phrase</p>
              </div>
            )}
