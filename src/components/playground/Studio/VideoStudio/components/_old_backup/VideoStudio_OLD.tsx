...
const VideoStudio: React.FC = () => {
  ...
  const handleCreateCanvasProject = async () => {
    try {
      ...
      if (result.success && result.project) {
        ...
        // Navigate to canvas
        navigate(`/overview/studio/video/canvas/${result.project.id}`);
  ...
  const handleLoadProject = async (project: VideoProject) => {
    try {
      ...
      // Navigate to canvas with project ID
      navigate(`/overview/studio/video/canvas/${project.id}`);
  ...
  <div className={`w-full flex flex-col relative ${isCanvasOpen ? 'h-screen pt-14' : 'h-full'}`}>
       ...
       {isCanvasOpen && (
           ...
           <RightPanel
            ...
            >
              <div className="space-y-4">
                <div className="text-white/70 text-sm">
                  <h4 className="font-medium mb-2">Video Properties</h4>
                </div>
              </div>
           </RightPanel>
            ...
            >
              <div className="h-full w-full overflow-hidden">
                <VideoTimeline
                  ...
                  onStateChange={(snapshot) => setTimelineSnapshot(snapshot)}
        ...
        {/* Canvas Video Interface Settings Modal (always available when triggered) */}
        {isProjectSettingsOpen && (
          <div className="fixed inset-0 z-[1100] flex items-center justify-center">
            <div className="absolute inset-0 bg-black/60" onClick={() => setIsProjectSettingsOpen(false)} />
            <div className="relative z-[1101] w-full max-w-2xl mx-4 border border-[#242426] bg-[#141416] shadow-2xl" style={{ borderRadius: `${innerCornerRadiusPx}px` }}>
              <div className="p-6 border-b border-[#242426] bg-[#18181a] rounded-t-[inherit]">
                <h3 className="text-xl font-semibold text-white">Canvas Video Interface Settings</h3>
                <p className="text-white/60 text-sm mt-1">Configure your canvas video generation and playback settings.</p>

                {userCredits >= 0 && (
                  <div className="mt-3 text-sm text-white/70 flex items-center justify-between">
                    <span>Your credits: <span className="font-semibold text-white">{userCredits}</span></span>
                    <span className="text-xs text-white/50 uppercase tracking-wide">Credits used on render</span>
                  </div>
                )}
              </div>
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-white/70 mb-1">Generation Steps</label>
                  <input
                    type="number"
                    min={1}
                    max={100}
                    ...
                    className="w-full bg-[#1b1b1d] border border-[#2c2c2f] rounded-lg px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#3a3a40]"
                  />
                  <p className="text-xs text-white/40 mt-1">Higher steps = better quality, slower generation</p>
                </div>
                <div>
                  <label className="block text-sm text-white/70 mb-1">Frame Rate (FPS)</label>
                  <input
                    type="number"
                    min={1}
                    max={120}
                ...
                <div>
                  <label className="block text-sm text-white/70 mb-1">Duration (seconds)</label>
                  <input
                    type="number"
                    min={1}
                    max={60}
              ...
              <div className="flex items-center justify-between p-6 border-t border-[#242426] bg-[#18181a] rounded-b-[inherit]">
                <div className="text-white/60 text-sm flex flex-col gap-1">
                  <div>{duration}s @ {fps}fps • {quality} quality • {aspectRatio} ratio • {steps} steps</div>
                  <div className="text-xs">
                    {currentProject && (
                      <span className="text-white/70">✓ Editing: {currentProject.title}</span>
                    )}
                    {isSavingProject && (
                      <span className="text-white/50 ml-2">💾 Saving...</span>
                    )}
                  </div>
                </div>
                <div className="flex gap-2">
                  <button
                    ...
                    className="px-4 py-2 rounded-lg bg-[#1f1f21] border border-[#2c2c2f] text-white hover:border-[#3a3a40]"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="px-4 py-2 rounded-lg bg-white text-black font-semibold hover:bg-gray-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-md flex items-center gap-2"
                  >
                    {isCreatingProject ? (
                      <>
                        <div className="inline-block w-4 h-4 border-2 border-black/20 border-t-black rounded-full animate-spin" />
                        Creating Project...
                      </>
                    ) : (
                      <span>Create Canvas Project</span>
                    )}
                  </button>
                </div>
              </div>
