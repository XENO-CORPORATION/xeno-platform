...
const ProjectsPage: React.FC = () => {
  ...
  const renameSelected = async () => {
    if (!selected) return;
    const workspaceId = activeWorkspace?.id;
    const next = window.prompt('Project name', selected.name)?.trim();
    if (!next || next === selected.name) return;
    setBusy(selected.id);
    try {
      const result = await updateProject(selected.id, { name: next });
      if (activeWorkspaceId.current !== workspaceId) return;
      setProjects((current) => current.map((item) => item.id === selected.id ? result.project : item));
    } catch (cause) { if (activeWorkspaceId.current === workspaceId) setError(cause instanceof Error ? cause.message : 'Project update failed.'); }
    finally { if (activeWorkspaceId.current === workspaceId) setBusy(''); }
  };

  const archiveSelected = async () => {
    if (!selected || !window.confirm(`Archive “${selected.name}”? Its active schedules will be paused.`)) return;
    const workspaceId = activeWorkspace?.id;
    setBusy(selected.id);
    try {
      await archiveProject(selected.id);
      if (activeWorkspaceId.current !== workspaceId) return;
      setProjects((current) => current.filter((item) => item.id !== selected.id));
      navigate('/overview/projects');
    } catch (cause) { if (activeWorkspaceId.current === workspaceId) setError(cause instanceof Error ? cause.message : 'Project archive failed.'); }
    finally { if (activeWorkspaceId.current === workspaceId) setBusy(''); }
  };
