...
const SettingsPage: React.FC = () => {
  ...
  const changePassword = async () => { setError(''); if (!currentPassword || newPassword.length < 6 || newPassword !== confirmPassword) { setError('Enter the current password and matching new passwords of at least 6 characters.'); return; } setSaving(true); try { const result = await authService.changePassword(currentPassword, newPassword); if (!result.success) throw new Error(result.error || 'Password update failed'); setPasswordOpen(false); setCurrentPassword(''); setNewPassword(''); setConfirmPassword(''); if (result.signedOut) { logout?.(); navigate('/login'); return; } setNotice(result.message || 'Password changed. Other devices have been signed out.'); loadSessions(); } catch (err) { setError(err instanceof Error ? err.message : 'Password update failed'); } finally { setSaving(false); } };
