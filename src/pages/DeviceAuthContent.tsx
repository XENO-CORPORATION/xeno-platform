...
const DeviceAuthContent = () => {
  ...
  return (
    <>
      <header className={`flex items-center justify-between p-6 lg:px-12 xl:px-20 lg:pt-12 xl:pt-16 transition-all duration-500 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`} style={{ transitionDelay: '0.1s' }}>
      ...
      <div className="flex-1 flex flex-col px-6 pb-12 lg:px-12 xl:px-20 pt-20 lg:pt-20 xl:pt-28">
        <div className={`w-full max-w-[400px] mx-auto transition-all duration-700 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: '0.15s' }}>
          <div className="mb-6 flex items-center gap-3 rounded-xl border border-white/[0.08] bg-white/[0.03] p-3">
            <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg text-sm font-bold" style={{ backgroundColor: `${authApp.accent}22`, color: authApp.accent }}>
          ...
          {status === 'connected' ? (
            <div className="rounded-xl border border-emerald-400/30 bg-emerald-400/[0.08] px-4 py-5 text-center">
            ...
            <form onSubmit={verify}>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  autoFocus
                  inputMode="text"
                  className="w-full rounded-xl border border-white/[0.08] bg-white/[0.02] py-3 pl-10 pr-3 text-center font-mono text-lg tracking-[0.3em] text-white placeholder:tracking-normal placeholder:text-white/20 focus:border-white/30 focus:outline-none"
                />
              </div>
              {status !== 'idle' && status !== 'verifying' && (
                <p className="mt-2 text-sm text-red-400">{status}</p>
              )}
              <button type="submit" disabled={status === 'verifying'} className="mt-5 w-full rounded-xl bg-white py-3 text-sm font-semibold text-black transition-colors hover:bg-white/90 disabled:opacity-60">
